...
func _initScene(_args = []):
	...
	for tag in craftableTag:
		if craftableTag[tag] == true:
			sellingItemsTags.append(int(tag))
...
func sort_stock(a, b):
	if sellItemsData[a]["nanoCost"] < sellItemsData[b]["nanoCost"]:
		return true
	return false
...
func _react(_action: String, _args):
	...
	if(_action == "buy"):
		...
		GM.pc.getInventory().removeXOfOrDestroy("NanoCore",sellItemsData[itemID]["nanoCost"])
		# GM.pc.addCredits(-sellItemsData[itemID]["price"] * amount)
		GM.pc.addSkillExperience("NanoENGR", 5*amount)
...
func onInventoryItemSelected(item: ItemBase):
	...
	if(state == "buymenu"):
		...
		var itemPrice = itemData["nanoCost"]

		var credits = GM.pc.getInventory().getAmountOf("NanoCore")
		print(credits)
		...
		for amount in amounts:
			...
			if(credits >= newPrice):
				addButton("Craft "+(str(newAmount))+" for "+str(newPrice)+" cores", "Amount: "+str(newAmount)+"\nCost: "+str(newPrice)+" cores", "buy", [item.id, amount])
			else:
				addDisabledButton("Craft "+(str(newAmount))+" for "+str(newPrice)+" cores", "Amount: "+str(newAmount)+"\nCost: "+str(newPrice)+" cores\nYou don't have enough cores")
	if(state == "sellmenu"):
